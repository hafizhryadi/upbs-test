...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $monthQuery = $dbDriver === 'sqlite' ? 'CAST(strftime("%m", trx_date) AS INTEGER)' : 'MONTH(trx_date)';
    ...
    public function requestReport()
    {
        ...
        $dbDriver = \Illuminate\Support\Facades\DB::connection()->getDriverName();
        $monthQuery = $dbDriver === 'sqlite' ? 'CAST(strftime("%m", created_at) AS INTEGER)' : 'MONTH(created_at)';

        $stats = \App\Models\Request::selectRaw('
            ' . $monthQuery . ' as month,
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $months[] = [
                'month_number' => $m,
                'month_name' => $date->locale('id')->translatedFormat('F'),
                'total_disetujui' => $stat ? $stat->total_disetujui : 0,
                'total_ditolak' => $stat ? $stat->total_ditolak : 0,
                'total_pending' => $stat ? $stat->total_pending : 0,
                'trx_count' => $stat ? $stat->trx_count : 0,
            ];
        }
        
        return view('reports.requests', compact('months', 'year'));
    }

    public function downloadRequestReport(string $id)
    {
        $year = request('year', date('Y'));
        $month = intval($id);
        ...
        $requests = \App\Models\Request::with(['variety'])
            ->whereYear('created_at', $year)
            ->whereMonth('created_at', $month)
            ->orderBy('created_at', 'asc')
            ->get();
            
        $pdf = Pdf::loadView('reports.pdf_requests', compact('requests', 'monthName', 'year'));
        $pdf->setPaper('A4', 'landscape');
        
        $pdfFileName = "Laporan_Permohonan_{$monthName}_{$year}.pdf";
