                                ...
                                <tbody>
                                    @forelse($stocks as $index => $stock)
                                        <tr>
                                            ...
                                            <td class="py-3 px-4">
                                                <div class="fw-bold text-dark">{{ $stock->variety->name ?? 'Unknown' }}
                                                </div>
                                            </td>
                                            <td class="py-3 px-4">
                                                {{ \Carbon\Carbon::parse($stock->expiry_date)->format('d M Y') }}
                                                @php
                                                    $statusData = \App\Models\Inventory::getStatusData(
                                                        $stock->expiry_date,
                                                    );
                                                @endphp
                                                @if ($statusData['status'] != 'safe')
                                                    <span
                                                        class="badge bg-{{ $statusData['badge'] }} {{ $statusData['badge'] == 'warning' ? 'text-dark' : '' }} ms-2">
                                                        {{ $statusData['label'] }}
                                                    </span>
                                                @endif
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-5 text-muted">Belum ada data stok
                                                benih.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
