                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="alert alert-warning">
                            <i class="bi bi-exclamation-triangle"></i> Perhatian: Mengedit jumlah atau tipe transaksi akan
                            mempengaruhi stok saat ini. Pastikan perubahan sudah benar.
                        </div>
                        <form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="trx_date" name="trx_date"
                                    value="{{ $transaction->trx_date }}" required>
                            </div>

                            <div class="mb-3">
                                <label for="variety_id" class="form-label">Varietas</label>
                                <input type="text" class="form-control" value="{{ $transaction->variety->name }}"
                                    disabled>
                                <input type="hidden" name="variety_id" value="{{ $transaction->variety_id }}">
                                <div class="form-text">Varietas tidak dapat diubah saat edit. Hapus dan buat baru jika salah
                                    varietas.</div>
                            </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="category" name="category" required>
                                        @foreach (['penjualan', 'diseminasi'] as $cat)
                                            <option value="{{ $cat }}"
                                                {{ $transaction->category == $cat ? 'selected' : '' }}>{{ ucfirst($cat) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
